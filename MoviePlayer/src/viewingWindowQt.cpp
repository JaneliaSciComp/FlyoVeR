...
ViewingWindowQt::ViewingWindowQt( int nCameras, std::vector< GraphWidget* > g, 
	...
	_output_treadmill_data( false ), _use_reduced_output( false ), _custom_widget_enabled( false ),
	_movie_mode( false ), _recording( false ), _capture_callback( 0 )
{
                                    ...
                                    bool trace_mode )
{
   ...
   if ( !trace_mode )
	   connect( &_timer, SIGNAL( timeout() ), this, SLOT( tracePaintEvent() ) );
   else
	...
	for ( int i = 0; i < frame_count; i++ )
	{
		...
		if ( _model->physics_enabled() )
		{
			...
			_model->step_simulation( _currSimTime - _prevSimTime, 
			                         2, frameTime / (double)(2 * frame_count));
...
//			std::cout << "dist - " << tv.length() << " " << frameTime << std::endl;
			...
			outputEvent( command, float_command, false, _currSimTime );
		}
	}
...
void
ViewingWindowQt::tracePaintEvent()
{ 
	double frameTime;
	osg::Vec3d eye, center, up;
	...
	// If not recording we increment then draw
	if ( !_recording )
		_model->update_time_index();
...
*/	
	...
	// But, if we are recording, we want to pick up the current frame before
	// incrementing
	if ( _recording )
		_model->update_time_index();
}
...
void
ViewingWindowQt::setUpSlaves( CameraTraceUpdateCallback* ct )
{
	...
	if ( _recording )
	{
		...
		_capture_callback = new WindowCaptureCallback( mode, position, readBuffer, 
		                                               _record_directory, _file_prefix, ct );
		addCallbackToViewer( *this, _capture_callback );
	}
	else
		if ( _capture_callback )
		{
			addCallbackToViewer( *this, _capture_callback, true );
			_capture_callback = 0;
		}
}
                                               ...
                                               int starting_display )
{
   ...
   double aspectRatioScale = ( (float)_width / (float)_height ) * 
	   ( (numCameras == 1 || _multipleScreens) ? 1.0 : (double)numCameras - 1 );
   double translate_x = double(numCameras)-1 + i * -2.0;
   double theta = 0.0;
