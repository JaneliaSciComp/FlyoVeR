...
int main(int argc, char* argv[])
{
	...
	wBuffer[0]=254;
	wBuffer[1]=0;
	ftStatus = FT_Write(ftHandle,wBuffer,2,&txBytes);
	Sleep(20);
	...
	wBuffer[1]=0;
	ftStatus = FT_Write(ftHandle,wBuffer,2,&txBytes);
	ftStatus = FT_Read(ftHandle,rBuffer,50,&rxBytes);
	...
	while(1){
		...
		ftStatus = FT_Read(ftHandle,rBuffer,200*12,&rxBytes);

		if (ftStatus != FT_OK || rxBytes!=2400){
