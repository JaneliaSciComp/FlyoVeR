// Avoid boost::multi-array warnings
// C:\Boost\include\boost-1_61\boost/multi_array/subarray.hpp(358): warning C4459: declaration of 'extents' hides global declaration
...
compute_frame_buffer_size( float frame_rate, int interval, boost::circular_buffer< T >* buffer, T* var, T const& zero_var )
{
	...
	if ( frame_rate > 0.f )
	{
		unsigned int num_frames = interval / ( frame_rate * 1000 );
		...
		if ( num_frames != buffer->size() )
		{
			if ( num_frames < buffer->size() )
			{
				...
				for( unsigned int i = 0; i < buffer->size() - num_frames; i++ )
				{
					t_var += *front;
					front++;
				}
...
{
	// return; // TODO temporary short circuit
	osg::Vec3f worldPt( worldTrans.getOrigin()[0],
	                    worldTrans.getOrigin()[1],
	                    worldTrans.getOrigin()[2] );
	...
	// This line seems to be required, even with physics disabled.
	// Otherwise, display is black CMB
	_track_node->setMatrix( matrix );
}
...
CameraMotionUpdateCallback::CameraMotionUpdateCallback( btRigidBody* body,
	...
	_interval_for_intermediary_smoothing( 32 ),
...
//			std::cout << currentPosition << ", " << targetPosition << ", " << targetPosition - _startPos << std::endl;
				...
				btScalar stepLengthSquared = step.length2();
