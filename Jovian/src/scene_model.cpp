...
void
Scene_Model::set_enable_fog ( bool on_or_off )
{
  _fog_enabled = on_or_off;
  // Only if a Collada model is loaded
  if ( _state_sets[ _active_model ] )
  {
    if ( on_or_off )
      _state_sets[ _active_model ]->setMode ( GL_FOG, osg::StateAttribute::ON );
    else
      _state_sets[ _active_model ]->setMode ( GL_FOG, osg::StateAttribute::OFF );
  }
}

void
Scene_Model::set_fog_color( osg::Vec4 diff )
{
	_precipitationEffect->getFog()->setColor( diff );
}

void
Scene_Model::set_fog_mode( int mode )
{
	switch( mode )
	{
		case 0:
			_precipitationEffect->getFog()->setMode( osg::Fog::LINEAR );
			break;
		case 1:
			_precipitationEffect->getFog()->setMode( osg::Fog::EXP );
			break;
		case 2:
			_precipitationEffect->getFog()->setMode( osg::Fog::EXP2 );
			break;
	}
}

void
Scene_Model::set_fog_density( double density )
{
	_precipitationEffect->getFog()->setDensity( density );
}

void
Scene_Model::set_fog_start( int pos )
{
	_precipitationEffect->getFog()->setStart( static_cast< float >( pos ) );
}

void
Scene_Model::set_fog_end( int pos)
{
	_precipitationEffect->getFog()->setEnd( static_cast< float >( pos ) );
}

void
Scene_Model::set_use_radial_fog( bool do_use_radial_fog )
{
	_precipitationEffect->getFog()->setUseRadialFog(do_use_radial_fog);
}
...
Scene_Model::update_model_index()
{
	...
	_state_sets.push_back( 0 );
	_camera_callback.push_back( 0 );
	_crude_obstacle_visitors.push_back( new CrudeObstacleVisitor() );
	_punishment_region_visitors.push_back( new PunishmentRegionVisitor() );
...
Scene_Model::set_export_filename( QString& filename )
{
	...
	if ( _out_file[ _active_model ] != 0 )
		_out_file[ _active_model ]->close();

	_out_file[ _active_model ] = out_file;
...
//   	root->addChild( _precipitationEffect.get() );
   	root->getOrCreateStateSet()->setAttributeAndModes( _precipitationEffect->getFog() );

   	_state_sets.back() = root->getOrCreateStateSet();
   	if ( !_fog_enabled )
		_state_sets.back()->setMode( osg::StateAttribute::FOG, osg::StateAttribute::OFF );
	...
	else
	{
		...
		// CMB process with no-physics visitor
		CrudeObstacleVisitor & obstacles = *(_crude_obstacle_visitors.back());
		nodeDB->accept(obstacles);
		obstacles.cullObstacles();

		PunishmentRegionVisitor & regions = *(_punishment_region_visitors.back());
		nodeDB->accept(regions);
		regions.cullObstacles();
		// Populuate ReinforcementRegionCategories in PunishmentRegionVisitor
		std::vector<CrudeVRObstacle>::const_iterator oi;
		// Investigate the category label in every obstacle found
		for (oi = regions.relevant_obstacles.begin(); oi != regions.relevant_obstacles.end(); ++oi) {
			const CrudeVRObstacle& ob = *oi;
			const std::string& label = ob.category_label;
			// Create a new category, if needed
			if (regions.reinforcementCategories.find(label) == regions.reinforcementCategories.end()) {
				regions.reinforcementCategories[label] = ReinforcementCategory(label);
			}
		}

		/*
		for ( std::vector<CrudeVRObstacle>::const_iterator i = obstacles.all_obstacles.begin();
			i != obstacles.all_obstacles.end(); i++) 
		{
			CrudeVRObstacle ob = *i;
			// std::cerr << "COV: " << ob.name << std::endl;
		}
		for ( std::vector<CrudeVRObstacle>::const_iterator i = obstacles.relevant_obstacles.begin();
			i != obstacles.relevant_obstacles.end(); i++) 
		{
			CrudeVRObstacle ob = *i;
			// std::cerr << "COV2: " << ob.name << std::endl;
		} */
        ...
        QFileInfo coords_info(filename);
        QString coords_path = coords_info.path() + "/" + coords_info.completeBaseName() + ".coords";
		coord_file.open(coords_path.toLatin1().data());
	...
	//	root_state_set->setAttributeAndModes( _precipitationEffect->getFog() );
