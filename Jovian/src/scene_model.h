...
public:
	...
	osg::ref_ptr< CrudeObstacleVisitor > crudeObstacleVisitor() { return _crude_obstacle_visitors[ _active_model ]; }
	osg::ref_ptr< PunishmentRegionVisitor > punishmentRegionVisitor() { return _punishment_region_visitors[ _active_model ]; }
	...
	osg::Vec3 player_location_in_world() { return _homeOffset[ _active_model ]->getWorldMatrices().back().getTrans(); }
	bool fog_enabled() const { return _fog_enabled; }
	const osg::Fog * getFog() const { 
		if (! _precipitationEffect) return nullptr;
		return _precipitationEffect->getFog(); 
	};
	...
	void set_enable_fog( bool on_or_off );
	void set_fog_color( osg::Vec4 diff );
	void set_fog_mode( int mode );
	void set_fog_density( double density );
	void set_average_framerate( float rate );
	void set_fog_start( int pos );
	void set_fog_end( int pos);
	void set_use_radial_fog( bool on_or_off );
	...
	std::ostream * logStream() {return _out_file[ _active_model ];}
...
private:
	...
	std::vector<osg::StateSet*> _state_sets;
	std::vector< osg::ref_ptr<CrudeObstacleVisitor> > _crude_obstacle_visitors;
	std::vector< osg::ref_ptr<PunishmentRegionVisitor> > _punishment_region_visitors;

	bool _first, _tracking_initialized, _fog_enabled;
