                        ...
                        osg::Vec2 lower_left, int size, int offset )
{
	...
	// add to this camera a subgraph to render
	{
		...
		stateset->setMode( GL_LIGHTING, osg::StateAttribute::OFF );
		{
			...
			osg::Geometry* geom_cyan = new osg::Geometry;
			osg::Geometry* geom_red = new osg::Geometry;
			...
			geom_cyan->setVertexArray( vertices );
			geom_red->setVertexArray( vertices );
			...
			geom_cyan->setNormalArray( normals );
			geom_cyan->setNormalBinding( osg::Geometry::BIND_OVERALL );
			geom_red->setNormalArray( normals );
			geom_red->setNormalBinding( osg::Geometry::BIND_OVERALL );
			...
			colors = new osg::Vec4Array;
			colors->push_back( osg::Vec4( 0.0f, 1.0, 1.0f, 1.0f ) );
			geom_cyan->setColorArray( colors );
			geom_cyan->setColorBinding( osg::Geometry::BIND_OVERALL );

			colors = new osg::Vec4Array;
			colors->push_back( osg::Vec4( 1.0f, 0.0, 0.0f, 1.0f ) );
			geom_red->setColorArray( colors );
			geom_red->setColorBinding( osg::Geometry::BIND_OVERALL );
			...
			geom_cyan->addPrimitiveSet( new osg::DrawArrays( GL_QUADS, 0, 4 ) );
			geom_red->addPrimitiveSet( new osg::DrawArrays( GL_QUADS, 0, 4 ) );
			...
			geode = new osg::Geode();
			geode->addDrawable( geom_cyan );
			switch_node->addChild( geode );

			geode = new osg::Geode();
			geode->addDrawable( geom_red );
			switch_node->addChild( geode );
		}
