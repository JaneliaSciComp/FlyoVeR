...
// Avoid Qt compile warning
#pragma warning(disable: 4512)
// f:\qt\4.7.4vc10x86\include\qtcore\../../src/corelib/global/qglobal.h(2275): warning C4512: 'QForeachContainer<T>' : assignment operator could not be generated
...
{
	GraphWidget* graph = source->graph_widget();
	...
	{
		foreach( Edge* e, edges_to_delete )
			scene()->removeItem( e );
	...
	if ( s_row == d_row )
	{
		for ( int i = 0; i < graph->nodes.size(); i++ )
		{
			...
			Edge* edge = nullptr;
			foreach( Edge* e, v[s_col]->edges() )
			{
				if ( e->source == v[s_col] && e->dest == v[d_col] )
					edge = e;
			}
			
			if (edge != nullptr) { // Should not happen...
	...
	else 	if ( s_col == d_col )
	{
		...
		{
			Edge* edge = nullptr;
			foreach( Edge* e, e_row[i]->edges() )
			{
				if ( e->source == e_row[i] && e->dest == e_col[i] )
					edge = e;
			}
			
			if (edge != nullptr) { // avoid compile warning
				QPointF vec = (edge->destPoint - edge->sourcePoint) * frac;
